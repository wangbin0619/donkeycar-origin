...
class EV3:
    ...
    def send_system_cmd(self, cmd: bytes, reply: bool=True) -> bytes:
        ...
        self._lock.acquire()
#        print("send_system_cmd lock acquired : _msg_cnt {}".format(self._msg_cnt))
        if self._msg_cnt < 65535:
            self._msg_cnt += 1
        else:
            self._msg_cnt = 1
        msg_cnt = self._msg_cnt
        self._lock.release()
#        print("send_system_cmd lock released : _msg_cnt {}".format(self._msg_cnt))
        cmd = b''.join([
    ...
    def _wait_for_system_reply(self, counter: bytes) -> bytes:
        ...
        """
        self._lock.acquire()
#        print("_wait_for_system_reply lock acquired : counter {}".format(counter))
        reply = self._get_foreign_reply(counter)
        if reply:
#            print("_wait_for_system_reply lock released (already in buffer) : counter {}".format(counter))
            self._lock.release()
            if reply[4:5] != _SYSTEM_REPLY:
                raise SysCmdError("error: {:02X}".format(reply[6]))

            self._lock.release()
#            print("error: {:100X} release lock".format(reply))
            return reply
        ...
        while True:
            ...
            if self._protocol in [BLUETOOTH, WIFI]:
                reply = self._socket.recv(1024)
 #               print("_wait_for_system_reply read socket BT and WIFI: {} ()".format(reply))
            else:
                reply = bytes(self._device.read(_EP_IN, 1024, 0))
 #              print("_wait_for_system_reply read socket OTHER: {} ()".format(reply))
            # pylint: enable=no-member
            len_data = struct.unpack('<H', reply[:2])[0] + 2
            reply_counter = reply[2:4]
            if self._verbosity >= 1:
...
#                print("_wait_for_system_reply lock released (already in buffer) : counter {}".format(counter))
    ...
    def _put_foreign_reply(self, counter: bytes, reply: bytes) -> None:
        ...
        print("_put_foreign_reply counter: {} reply: {} _foreign: {}".format(counter,reply,self._foreign))
    ...
    def _get_foreign_reply(self, counter: bytes) -> bytes:
        ...
        if counter in self._foreign:
            ...
            print("_get_foreign_reply counter: {} reply: {} _foreign: {}".format(counter, reply, self._foreign))
            return reply
        else:
            return None
# pylint: enable=too-many-instance-attributes
